...
const setLearningPath = async (req, res) => {
    console.log("The request is:", req.body);
    try {
        const createLearning = new createLearningModel();
        const learningPathFeedback = await createLearning.setUsersLearningPath(req.body);
        res.status(200).json(learningPathFeedback);
    } catch (error) {
        console.log("It seems there was an error setting the learning path you selected", error);
        res.status(500).json({ message: 'Something went a bit wrong setting your learning path.' });
    }
}
